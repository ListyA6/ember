...
(function (App) {
  ...
  function openProgramPicker() {
    const keys = ['upperA', 'upperB', 'lowerA', 'lowerB', 'rest'];
  ...
  function renderRest(page) {
    ...
    const walkBtn = el('button', { class: 'btn block mt-3', html: icon('walk', 19) + '<span>Log a walk</span>' });
    walkBtn.addEventListener('click', () => { haptic(12); App.flows.logActivity('walk'); });
  ...
  function renderActive(page, program, sess) {
    ...
    finish.addEventListener('click', async () => {
      ...
      if (!c) return;
      if (!c.photo) {
        const ok = await confirm({ title: 'Finish without a photo?', message: 'A gym photo makes the feed entry land harder.', confirmText: 'Finish' });
        if (!ok) return;
      }
      const act = store.finishSession({ photo: store.currentSession() && store.currentSession().photo });
      clearTickers();
  ...
  function render(root, params) {
    ...
    const program = store.programFor();
    const sess = store.currentSession();
    const activeToday = sess && sess.date === store.today();
