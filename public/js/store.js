...
(function (App) {
  ...
  const KEY = 'ember_v1';
  const START = '2026-05-21';      // fitness restart anchor (from brain/fitness.md)
  ...
  const PROGRAMS = {
    upperA: { name: 'Upper A · Push', color: 'upper', ex: [
    ...
    upperB: { name: 'Upper B · Pull', color: 'upper', ex: [
  ...
  function freshState() {
    return {
      ...
      goals: { stepsPerDay: 10000, workoutsPerWeek: 4, startDate: START, endDate: GOAL_END },
      sync: { url: 'https://fit.sidestudio.id/api.php', token: '', enabled: false, lastAt: null }
    };
  }
  ...
  function load() {
    try {
      ...
      return Object.assign(freshState(), raw, { meta: Object.assign(freshState().meta, raw.meta) });
  ...
  const store = {
    D, PROGRAMS, DAY_PLAN, ACT, BADGES, START, GOAL_END,
    get state() { return S; },
    ...
    isTrainer() { return S.meta.role !== 'client'; },   // null/trainer => trainer-capable
    isClient() { return S.meta.role === 'client'; },
    ...
    weightSeries(pid) { return (S.weight[pid || S.me] || []).slice(); },
    latestWeight(pid) { const a = S.weight[pid || S.me] || []; return a.length ? a[a.length - 1] : null; },
    ...
    programFor(date) {
      date = date || new Date();
      const dk = D.key(date);
      if (S.overrides[dk] && PROGRAMS[S.overrides[dk]]) return Object.assign({ key: S.overrides[dk] }, PROGRAMS[S.overrides[dk]]);
      const k = DAY_PLAN[date.getDay()] || 'rest';
      return Object.assign({ key: k }, PROGRAMS[k]);
    },
    ...
    startSession(programKey) {
      const p = programKey ? Object.assign({ key: programKey }, PROGRAMS[programKey]) : store.programFor(new Date());
    ...
    detectPRs(activity) {
      ...
      (activity.sets || []).forEach(s => {
        ...
        if (e > b.e1rm + 0.01 && e > cur.e1rm) { /* est-1rm pr (only if not already a heavier weight pr same ex) */ }
    ...
    activeDays(pid) {
      pid = pid || S.me;
      const set = {};
      store.activitiesFor(pid).forEach(a => set[a.date] = true);
    ...
    weekBars(pid) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const count = store.activitiesFor(pid).filter(a => a.date === dk).length;
    ...
    heatmap(pid, days) {
      ...
      const acts = {}; store.activitiesFor(pid).forEach(a => acts[a.date] = (acts[a.date] || 0) + 1);
    ...
    importJSON(str) {
      ...
      if (obj.kind === 'ember-profile') return store.importProfile(obj);
      // full backup restore
      S = Object.assign(freshState(), obj); save(); return { restored: true };
    },
    importProfile(obj) {
    ...
    async syncNow() {
      if (!S.sync.enabled || !S.sync.url) throw new Error('Sync disabled');
      const r = await fetch(S.sync.url + '?action=sync', {
        method: 'POST',
        headers: { 'Authorization': 'Bearer ' + S.sync.token, 'Content-Type': 'application/json' },
        body: JSON.stringify({ activities: store.activitiesFor(S.me), walks: S.walks[S.me] || {}, weight: S.weight[S.me] || [] })
      });
      const j = await r.json();
      if (!r.ok || !j.ok) throw new Error(j.error || ('HTTP ' + r.status));
      S.sync.lastAt = Date.now(); save();
      return j;
    },
